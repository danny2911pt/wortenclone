...
function listacompras(){
	...
	while($items = mysqli_fetch_array($lercarrinho)){
		echo '<div class="itemlista">
				...
				<table>
					<tr>
						<td><img style="width: 70%" src="img/'.$items["art_img"].'"></td>
						<td> '.$items["art_nome"].'</td>
		';
						if ($items["art_promo"] == 1)
                        {
                            echo '
                                <td style="text-decoration: line-through; width: 10%">'.$items["art_preco"].'€</td>
                                <td style="width: 10%">'.$items["art_precopromo"].'€</td>
                            ';
                        }
						else echo '<td>'.$items["art_preco"].'€</td>';
		...
		echo'
						...
						<td><input type="submit" name="atualizaritem" value="Atualizar"></td>
						<td><input type="submit" name="apagaritem" value="Remover"></td>
					</tr>
				</table>
				</form>
			</div>';
...
function talao()
{
	...
	';
		...
		while($itemtalao = mysqli_fetch_array($itemstalao))
        {
			...
			$valorp = $itemtalao["art_precopromo"] * $itemtalao["qta"];

			echo '
                <div class="itemtalao">
			        <div class="itemnome">'.$itemtalao["qta"].' x '.$itemtalao["art_nome"].'</div>
			';
				if($itemtalao["art_promo"] == 1)
				{
				    echo' <div class="itemvalor">'.$valorp.'€</div>';
                    $total = $total + $valorp;
				}
				else
				{
				    echo' <div class="itemvalor">'.$valor.'€</div>';
                    $total = $total + $valor;
				}
            echo '</div>';
        }
