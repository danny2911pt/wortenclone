		...
		echo '<a href="?n=10&art_id='.$artdestaque["art_id"].'">
			<div class="destaque">
				...
				<div class="precosdestaque">
					<div class="precoatual">'.$artdestaque["art_preco"].'€</div>
					<div class="precoanterior">'.$artdestaque["art_precopromo"].'€</div>
				</div>
				</div>
			</div></a>';
	}
...
function compracarrinho($meio){
	...
	if(!$_SESSION["id_login"]){
		...
		$hoje = date('Y-m-d');
		mysqli_query($conn,"UPDATE tb_cart SET cart_status = '1', meiopagamento = '$meio', data = '$hoje' WHERE cart_id = '$_SESSION[hash]'");
